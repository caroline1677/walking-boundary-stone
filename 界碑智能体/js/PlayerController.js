...
export class PlayerController {
  constructor({ world, cameraController, assets }) {
    ...
    this.radius = 0.3;
    this.bindEvents();
    this.formalModelPromise = this.loadFormalModel();
  ...
  update(dt) {
    ...
    const touch = window.__touchMove;
    let touchX = 0, touchZ = 0;
    let running = this.keys.has("ShiftLeft") || this.keys.has("ShiftRight");
    if (touch && touch.active) {
      touchX = touch.x;
      touchZ = touch.y;
      running = running || !!touch.run;
    }
    const x = Math.max(-1, Math.min(1,
      (this.keys.has("KeyD") || this.keys.has("ArrowRight") ? 1 : 0) - (this.keys.has("KeyA") || this.keys.has("ArrowLeft") ? 1 : 0) + touchX));
    const z = Math.max(-1, Math.min(1,
      (this.keys.has("KeyW") || this.keys.has("ArrowUp") ? 1 : 0) - (this.keys.has("KeyS") || this.keys.has("ArrowDown") ? 1 : 0) + touchZ));
    if (!x && !z) {
      if (this.model) {
        this.model.position.y *= 0.72;
        this.model.rotation.z *= 0.72;
        this.model.rotation.x *= 0.72;
      }
      return;
    }
    ...
    this.playWalkFeel(dt, running);
  }

  // 模型还没有骨骼动画时的过渡步态：轻微上下起伏 + 左右摆动，减轻"平移滑行"感
  playWalkFeel(dt, running) {
    if (this.model) {
      this._phase = (this._phase || 0) + dt * (running ? 11.5 : 8);
      this.model.position.y = Math.abs(Math.sin(this._phase)) * 0.05;
      this.model.rotation.z = Math.sin(this._phase) * 0.05;
      this.model.rotation.x = Math.sin(this._phase * 2) * 0.015;
    }
  }
}
