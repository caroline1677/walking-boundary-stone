...
export class UIManager {
  ...
  setLoading(text, progress = 0) {
    ...
    if (this._loadStart == null || progress < (this._lastProgress ?? 0)) this._loadStart = now;
    this._lastProgress = progress;
    this._loadInfo = { text, progress };
    this._renderLoading();
    if (!this._loadTicker) this._loadTicker = setInterval(() => this._renderLoading(), 500);
  }

  _renderLoading() {
    if (!this._loadInfo) return;
    const { text, progress } = this._loadInfo;
    const now = performance.now();
    const elapsed = (now - this._loadStart) / 1000;
    let suffix = "";
    if (progress > 0.06 && progress < 0.98 && elapsed > 1.5) {
      const remaining = Math.max(1, Math.round((elapsed / progress) - elapsed));
      suffix = remaining >= 60
        ? `（预计还需 ${Math.floor(remaining / 60)} 分 ${String(remaining % 60).padStart(2, "0")} 秒）`
        : `（预计还需 ${remaining} 秒）`;
    }
    this.loadingText.textContent = `${text}${suffix}`;
    this.loadingBar.style.width = `${Math.round(progress * 100)}%`;
    if (progress >= 1 && this._loadTicker) {
      clearInterval(this._loadTicker);
      this._loadTicker = null;
    }
  }
